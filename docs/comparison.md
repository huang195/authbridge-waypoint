...
| **Pattern** | Shared HTTP proxy (`HTTP_PROXY` env var) | Shared infrastructure (Istio waypoint ext_authz) | Per-pod sidecar (Envoy + ext_proc + iptables) | Per-pod sidecar (Go binary, forward + reverse proxy) |
| **Mesh required** | No | Istio ambient | No | No |
...
All four face the same fundamental TLS limitation: token exchange cannot be performed on HTTPS outbound traffic without TLS termination. For HTTPS destinations, the application must set the Authorization header itself. Waypoint has a partial workaround via egress gateway (infrastructure-managed TLS termination), which the others do not.
...
| **App change required** | `HTTP_PROXY` env var | None | None | `HTTP_PROXY` env var |
...
AuthBridge uses an admission webhook to inject sidecars automatically, so the developer-facing YAML is similar to Waypoint. The Shared Proxy approach uses a webhook to set `HTTP_PROXY` on agent pods.
...
| **Compromised app can steal exchange credentials** | No | Yes | Yes | Yes |
